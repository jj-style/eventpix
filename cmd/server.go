...
func newServerApp(cfg *config.Config, logger *zap.Logger, nc *nats.Conn, srv *http.Server) (*serverApp, func(), error) {
	app := &serverApp{
		server:      srv,
		thumbnailer: nil,
	}
	...
	if cfg.Nats.InProcess {
		thumbnailer, cleanup, err = initializeThumbnailerWithNats(cfg, logger, nc)
	}
	if err != nil {
		return nil, func() {}, err
	}
